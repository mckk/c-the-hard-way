...
int main (int argc, char *argv[])
{
  ...
  switch(action) {
    ...
    case 'g':
      if (id == -1) die("Need a valid id to get");

      Database_get(id);
      break;

    case 's':
      if (id == -1) die("Need a valid id to set");
      if (argc != 6) die("Need name and email to set");
    ...
    case 'd':
      if (id == -1) die("Need a valid id to delete");
    ...
    case 'f':
      if (argc != 4) die("Need a search term to search for");
